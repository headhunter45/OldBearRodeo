...
/**
 * Generates an array of fantasy names using the syllable patterns.
 * @param count Number of names to generate (default 10)
 * @param type 'male' | 'female' | 'random' (defaults to alternating or randomly choosing)
 */
export function nameGen(count = 10, type?: 'male' | 'female' | 'random'): string[] {
  const names: string[] = [];

  for (let i = 0; i < count; i++) {
    const rnd4 = Math.floor(Math.random() * nm7.length);
    const rnd5 = Math.floor(Math.random() * nm8.length);
    const surname = nm7[rnd4] + nm8[rnd5];
    ...
    if (gender === 'male') {
      ...
      names.push(nm4[rnd] + nm5[rnd2] + nm6[rnd3] + " " + surname);
    } else {
      ...
      names.push(nm1[rnd] + nm2[rnd2] + nm3[rnd3] + " " + surname);
    }
  }
...
/**
 * Generates a single random fantasy name.
 */
export function generateRandomName(type?: 'male' | 'female' | 'random'): string {
  return nameGen(1, type)[0];
}
